...
const sendMessage_1 = require("../../src/api/sendMessage");
...
describe('SendMessage API Tests', () => {
    ...
    test('should send message successfully - status 200', async () => {
        ...
        const response = await (0, sendMessage_1.sendMessage)({
    ...
    test('should return 400 for missing chatId', async () => {
        ...
        try {
            await (0, sendMessage_1.sendMessage)({ message: "Test" });
        }
        catch (error) {
    ...
    test('should return 400 for missing message', async () => {
        ...
        axios.post.mockRejectedValue(mockError);
        try {
            await (0, sendMessage_1.sendMessage)({ chatId: config_1.config.chatId });
        }
        catch (error) {
            const apiError = error;
            expect(apiError.response.status).toBe(400);
    ...
    test('should return error for unauthorized instance', async () => {
        ...
        axios.post.mockRejectedValue(mockError);
        try {
            await (0, sendMessage_1.sendMessage)({
                chatId: config_1.config.chatId,
